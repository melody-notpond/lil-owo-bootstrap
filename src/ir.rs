...
type IrScope<'a> = Vec<HashMap<&'a str, IrArgument>>;

mod scope {
    use super::*;

    pub fn get(scope: &[HashMap<&str, IrArgument>], var: &str) -> Option<IrArgument> {
...
fn ast_to_ir_helper<'a>(ast: Ast<'a>, scope: &mut IrScope<'a>, module: &mut IrModule, func: &mut IrFunction, block: &mut IrBasicBlock) -> Result<Option<usize>, IrError> {
    match ast {
        Ast::Number(float) => {
        ...
        Ast::Symbol(sym) => {
            let value = match scope::get(scope, sym) {
                Some(v) => v,
        ...
        Ast::SExpr(f, args) => {
            ...
            match *f {
                Ast::Symbol(v) if scope::get(scope, v).is_some() => {
                    let mut locals = vec![scope::get(scope, v).unwrap()];
                    for arg in args {
                ...
                Ast::Symbol("atom") => {
                    ...
                    for arg in args {
                        if let Ast::Symbol(atom) = arg {
                            scope.insert(atom, IrArgument::Atom(String::from(atom)));
                        }
                    }
                ...
                Ast::Symbol("func") => {
                    ...
                    scope.last_mut().unwrap().insert(name, IrArgument::Function(String::from(name)));
                    ...
                    for (i, formal) in formals.iter().enumerate() {
                        last_scope.insert(formal, IrArgument::Argument(i));
                    }
                ...
                Ast::Symbol("set") => {
                    ...
                    if scope.get(name).is_none() {
                        scope.insert(name, IrArgument::Local(value));
                    } else {
